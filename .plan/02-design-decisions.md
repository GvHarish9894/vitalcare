...
**Amendment (2026-07-05):** the implementation stores its snapshot in Drive's hidden `appDataFolder` (D-023), which the `drive.file` scope cannot reach — the requested scope is therefore **`https://www.googleapis.com/auth/drive.appdata`**. This is strictly *more* least-privilege than `drive.file`: it can only ever access the app's own hidden data folder, never any of the user's real Drive files, and it survives app reinstalls (so restore-after-reinstall works). The consent screen must list this scope.
**Rejected:** Full `drive` scope (over-broad for a backup app); using Google Sign-In as the app's identity (re-introduces the login gate we removed); a visible `drive.file` file in the user's Drive (loses reinstall-safe access and clutters their Drive).
...
## D-033 — Fluid balance (water intake + urine output) as a separate, sum-aggregated feature — `Settled`
